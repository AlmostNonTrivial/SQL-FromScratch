...
load_table_from_csv_sql(const char *csv_file, const char *table_name)
{
	...
	while (*current)
	{
		...
		char **fields = (char **)arena<query_arena>::alloc(sizeof(char *) * structure->columns.size());
		...
		char *field_buffer = (char *)arena<query_arena>::alloc(line_end - current + 1);
			...
			else if ((*current == ',' || current == line_end) && !in_quotes)
			{
				...
				if (field_count < structure->columns.size())
				{
					*write_pos = '\0';
					...
					while (field_len > 0 && (field_final[field_len - 1] == ' ' || field_final[field_len - 1] == '\t' ||
					...
					field_buffer = (char *)arena<query_arena>::alloc(line_end - current + 1);
					write_pos = field_buffer;
					field_start = current + 1;
				}
...
vmfunc_like(typed_value *result, typed_value *args, uint32_t arg_count)
{
	...
	std::string_view text(args[0].as_char(), strnlen(args[0].as_char(), type_size(args[0].type)));
	std::string_view pattern(args[1].as_char(), strnlen(args[1].as_char(), type_size(args[1].type)));
