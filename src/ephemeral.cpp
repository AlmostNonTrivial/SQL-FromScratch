 ...
 * All nodes are allocated in the query_arena. The tree is 'ephemeral' because
 * it doesn't persist between queries, as like other query_arena allocations
 * they are deallocated in bulk after the execution
 *
 * For visualizing the Red-Black tree algorithm see
 * https://www.cs.usfca.edu/~galles/visualization/RedBlack.html
 *
 * Node structure and data are allocated in a single block:
 * [node_struct][key_bytes][record_bytes] Like the b+tree, access to a part of a
 * record involves pointer arithmetic, but unlike the b+tree it's 1 key-record
 * pair per node.
 *
...
static inline int node_compare_key(const ephemeral_tree *tree, void *key,
                                   ephemeral_tree_node *node) {
  return type_compare(tree->key_type, key, GET_KEY(node));
}

static ephemeral_tree_node *alloc_node(ephemeral_tree *tree, void *key,
                                       void *record) {
...
static ephemeral_tree_node *tree_minimum(ephemeral_tree_node *node) {
...
static ephemeral_tree_node *tree_maximum(ephemeral_tree_node *node) {
...
static ephemeral_tree_node *tree_successor(ephemeral_tree_node *node) {
  if (node->right) {
...
static ephemeral_tree_node *tree_predecessor(ephemeral_tree_node *node) {
  if (node->left) {
...
static void rotate_left(ephemeral_tree *tree, ephemeral_tree_node *x) {
...
static void rotate_right(ephemeral_tree *tree, ephemeral_tree_node *x) {
...
static void insert_fixup(ephemeral_tree *tree, ephemeral_tree_node *z) {
...
static void transplant(ephemeral_tree *tree, ephemeral_tree_node *u,
                       ephemeral_tree_node *v) {
  if (!u->parent) {
    tree->root = v;
...
static void delete_fixup(ephemeral_tree *tree, ephemeral_tree_node *x,
                         ephemeral_tree_node *x_parent) {
...
static bool delete_node(ephemeral_tree *tree, ephemeral_tree_node *z) {
  if (!z) {
    return false;
  }
...
static ephemeral_tree_node *seek_eq(ephemeral_tree *tree, void *key) {
...
static ephemeral_tree_node *seek_ge(ephemeral_tree *tree, void *key) {
  ephemeral_tree_node *current = tree->root;
...
static ephemeral_tree_node *seek_gt(ephemeral_tree *tree, void *key) {
  ephemeral_tree_node *current = tree->root;
  ephemeral_tree_node *best = nullptr;
...
static ephemeral_tree_node *seek_le(ephemeral_tree *tree, void *key) {
  ephemeral_tree_node *current = tree->root;
  ephemeral_tree_node *best = nullptr;
...
static ephemeral_tree_node *seek_lt(ephemeral_tree *tree, void *key) {
  ephemeral_tree_node *current = tree->root;
  ephemeral_tree_node *best = nullptr;
...
ephemeral_tree et_create(data_type key_type, uint32_t record_size,
                         uint8_t flags) {
  ephemeral_tree tree = {};
  ...
  tree.allow_duplicates = true; // flags & 0x01;
  return tree;
}

void et_clear(ephemeral_tree *tree) {

  tree->root = nullptr;
  tree->node_count = 0;
}

bool et_insert(ephemeral_tree *tree, void *key, void *record) {
...
bool et_delete(ephemeral_tree *tree, void *key) {
  ephemeral_tree_node *current = tree->root;
...
bool et_cursor_seek(et_cursor *cursor, const void *key, COMPARISON_OP op) {
...
bool et_cursor_update(et_cursor *cursor, void *record) {
  ...
  memcpy(GET_RECORD(cursor->current, &cursor->tree), record,
         cursor->tree.record_size);
  return true;
}

static int
validate_node_recursive(const ephemeral_tree *tree, ephemeral_tree_node *node,
                        ephemeral_tree_node *expected_parent, void *min_bound,
                        void *max_bound,
  ...
  if (min_bound) {
    assert(node_compare_key(tree, min_bound, node) < 0 &&
           "BST violation: node smaller than min bound");
  }
  if (max_bound) {
    assert(node_compare_key(tree, max_bound, node) > 0 &&
           "BST violation: node larger than max bound");
  }
  ...
  int left_black_height =
      validate_node_recursive(tree, node->left, node, min_bound, key, visited);
  int right_black_height =
      validate_node_recursive(tree, node->right, node, key, max_bound, visited);
...
void et_validate(const ephemeral_tree *tree) {
  ...
  int black_height = validate_node_recursive(tree, tree->root, nullptr, nullptr,
                                             nullptr, visited);
...
static void
print_inorder_recursive(const ephemeral_tree *tree, ephemeral_tree_node *node,
                       bool &first, array<data_type, query_arena> *columns)
{
	if (!node)
	{
		return;
	...
	print_inorder_recursive(tree, node->left, first, columns);
	...
	data_type key_type = (columns && columns->size() > 0)
		? (*columns)[0]
		: tree->key_type;
	type_print(key_type, GET_KEY(node));
	...
	if (tree->record_size > 0)
	{
		...
		{
			// Use schema for pretty printing
			print_record_columns((uint8_t *)GET_RECORD(node, tree), columns, 1);
		}
		else
		{
	...
	print_inorder_recursive(tree, node->right, first, columns);
}
...
static void
print_tree_visual_helper(const ephemeral_tree *tree, ephemeral_tree_node *node,
                        ...
                        array<data_type, query_arena> *columns)
{
	...
	// Use columns[0] type if schema provided
	data_type key_type = (columns && columns->size() > 0)
		? (*columns)[0]
		: tree->key_type;
	type_print(key_type, GET_KEY(node));
	...
	if (node->left || node->right)
	{
		if (node->right)
		{
			print_tree_visual_helper(tree, node->right, child_prefix, false, columns);
		}
		if (node->left)
		{
			print_tree_visual_helper(tree, node->left, child_prefix, true, columns);
		}
	}
}

void
et_print(const ephemeral_tree *tree, array<data_type, query_arena> *columns)
{
	if (!tree || !tree->root)
	{
	...
	while (!queue.empty())
	{
		...
		data_type key_type = (columns && columns->size() > 0)
			? (*columns)[0]
			: tree->key_type;
		type_print(key_type, GET_KEY(nl.node));
		printf("]");
	...
	bool first = true;
	print_inorder_recursive(tree, tree->root, first, columns);
	printf("\n");

	printf("\nVisual Structure:\n");
	print_tree_visual_helper(tree, tree->root, "", true, columns);
	printf("====================================\n\n");
}
