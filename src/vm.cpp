...
vmcursor_rewind(vm_cursor *cur, bool to_end)
{
	...
	case BPLUS:
		return to_end ? bt_cursor_last(&cur->cursor.btree) : bt_cursor_first(&cur->cursor.btree);
	default:
		return false;
	}
}
...
vmcursor_step(vm_cursor *cur, bool forward)
{
	...
	case BPLUS:
		return forward ? bt_cursor_next(&cur->cursor.btree) : bt_cursor_previous(&cur->cursor.btree);
	default:
		return false;
	}
}
...
step()
{
	...
	case OP_Unpack: {
		...
		assert(type_is_dual(dual_val->type) && "Type to unpack must be a dual type");
